...
class FlashAttnSwizzle:
    ...
    def __call__(
        ...
        # TODO: create a swizzle with Swizzle<B=3, M=3, S=3>
        # TODO: create base row-major layouts for Q and KV (same as naive)
        # TODO: compose the swizzle with the base layouts to get sQ_layout and sKV_layout
        #
        # Hint:
        #   sw = cute.make_swizzle(?, ?, ?)
        #   base_Q  = cute.make_layout((BLOCK_Q, HEAD_DIM), stride=(HEAD_DIM, 1))
        #   sQ_layout = cute.make_composed_layout(sw, 0, base_Q)
        #   (same for KV)
        smem_k_block_size = 64  # since HEAD_DIM % 64 == 0
        ...
        # ---- shared memory struct (same as naive — same sizes, just layout changes) ----
        @cute.struct
        class SharedStorage:
    ...
    def kernel(
        ...
        # ============================================================
        # EVERYTHING BELOW IS IDENTICAL TO THE NAIVE KERNEL.
        # The swizzle is entirely contained in sQ_layout / sKV_layout.
        # All partition/copy/MMA ops see the same logical shape but
        # access physically remapped addresses — zero code changes here.
        # ============================================================
        ...
        for kv in range(num_kv_tiles):
            ...
            sVt_base = cute.make_layout(
                (HEAD_DIM, BLOCK_KV),
                stride=(1, HEAD_DIM)
            )
            sVt = cute.make_tensor(sV.iterator, sVt_base)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Flash Attention v2 — Swizzled CuteDSL (v3)")
    print("=" * 60)
    print()
